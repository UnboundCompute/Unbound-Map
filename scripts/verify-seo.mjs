...
const { flowCardHref } = await import('../lib/artifact-id.ts');

const artifact = documentMetadata(
  'Flow card · Unbound Map',
  'A pinned flow card.',
  { repository: 'acme/widget', revision: 'a'.repeat(40), bundle: 'b_12345678', flow: 'request.main', canonicalPath: flowCardHref('b_12345678', 'request.main') },
);
assert.deepEqual(artifact.robots, { index: false, follow: true });
assert.equal(artifact.alternates?.canonical, '/f/b_12345678/request.main?renderer=1');
