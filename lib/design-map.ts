...
export function isLachesisBundle(value: unknown): value is LachesisBundle {
  ...
  return candidate.format === 'lachesis-explorer-bundle'
    ...
    && (!graph.coverage || (typeof graph.coverage === 'object' && (!graph.coverage.scope || typeof graph.coverage.scope === 'string') && (!graph.coverage.included_nodes || (typeof graph.coverage.included_nodes === 'number' && Number.isInteger(graph.coverage.included_nodes) && graph.coverage.included_nodes >= 0)) && (!graph.coverage.indexed_nodes || (typeof graph.coverage.indexed_nodes === 'number' && Number.isInteger(graph.coverage.indexed_nodes) && graph.coverage.indexed_nodes >= 0)) && (!graph.coverage.limitations || (Array.isArray(graph.coverage.limitations) && graph.coverage.limitations.every((item) => typeof item === 'string'))) && (!graph.coverage.capabilities || (Array.isArray(graph.coverage.capabilities) && graph.coverage.capabilities.every((item) => typeof item === 'string')))));
}
