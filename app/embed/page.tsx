...
export default async function EmbedPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="embed-page"><header className="embed-header"><div><span className="embed-kicker">Design Map · HLD</span><h1>{repository} architecture</h1><p>Read the system shape before the source.</p></div><Link href={`/architecture${contextQuery ? `?${contextQuery}` : ''}`}>Open full guide <span aria-hidden="true">↗</span></Link></header><MapClient route="/embed" /><footer className="embed-footer"><span>{bundle ? 'Graph-backed bundle requested' : `Illustrative snapshot · ${illustrativeSnapshot.revision}`}</span><Link href={`/${contextQuery ? `?${contextQuery}` : ''}`}>About this map</Link></footer></main>;
}
