...
export default async function EmbedPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="embed-page"><a className="skip-link" href="#embed-content">Skip to map</a><header className="embed-header"><div><span className="embed-kicker">Design Map · HLD</span><h1>{repository} architecture</h1><p>Read the system shape before the source.</p></div><Link href={`/architecture${contextQuery ? `?${contextQuery}` : ''}`}>Open full guide <span aria-hidden="true">↗</span></Link></header><div id="embed-content" tabIndex={-1}><MapClient route="/embed" initialBundle={bundle} /></div><footer className="embed-footer"><span>{bundle ? 'Graph-backed bundle requested' : `Illustrative snapshot · ${illustrativeSnapshot.revision}`}</span><Link href={`/${contextQuery ? `?${contextQuery}` : ''}`}>About this map</Link></footer></main>;
}
