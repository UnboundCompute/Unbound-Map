...
export function HostedTrustGlossary({ bundleId, context, initialQuery = '' }: { bundleId: string; context: SharedSnapshotContext; initialQuery?: string }) {
  ...
  return <section className="hosted-trust" aria-label="Graph-backed trust evidence"><div className="trust-intro"><h2>Read the evidence, then inspect the obligation.</h2><p>This snapshot exports {findings.length.toLocaleString()} graph evidence item{findings.length === 1 ? '' : 's'}. Presence marks a place to investigate; it is not an adjudicated vulnerability or a security score.</p></div><div className="trust-tools"><label htmlFor="hosted-trust-search">Find evidence</label><input id="hosted-trust-search" type="search" value={query} onChange={(event) => updateQuery(event.target.value)} placeholder="Search exported path names…" /></div><div className="trust-result-count" role="status" aria-live="polite" aria-atomic="true">{filtered.length} of {findings.length} evidence items</div>{filtered.length ? <div className="trust-domain-list">{filtered.map((finding) => { const index = findings.indexOf(finding); const steps = finding.witness?.steps ?? []; const firstNode = steps[0] ? nodes.get(steps[0].node_id) : undefined; const lastNode = steps.at(-1) ? nodes.get(steps.at(-1)!.node_id) : undefined; const anchor = firstNode?.label ?? lastNode?.label ?? findingLabel(finding, index); const handoff = `/explore?${new URLSearchParams({ repository: bundle!.meta.repository, revision: bundle!.meta.revision, bundle: bundleId, anchor, ...(firstNode?.module ? { region: firstNode.module } : {}) }).toString()}`; return <article className="trust-domain hosted-trust-item" key={finding.finding_id ?? `${anchor}-${index}`}><div className="trust-domain-index"><span>{String(index + 1).padStart(2, '0')}</span><small>{finding.analysis?.confidence ? `confidence · ${finding.analysis.confidence}` : 'graph evidence'}</small></div><div><h2>{findingLabel(finding, index)}</h2><p className="trust-meaning">{finding.result_summary?.trim() || 'The exporter identified a source-backed path worth reading in context.'}</p><p className="trust-obligation"><strong>Interpretation</strong><span>Inspect the source and guards before deciding whether this represents a defect.</span></p>{steps.length > 0 && <div className="trust-families">{steps.map((step, stepIndex) => <code key={`${step.node_id}-${stepIndex}`}>{nodes.get(step.node_id)?.label ?? step.node_id}</code>)}</div>}<div className="trust-meta"><code>{firstNode?.file || 'source location unavailable'}{firstNode?.line ? `:${firstNode.line}` : ''}{lastNode && lastNode.id !== firstNode?.id ? ` → ${lastNode.file}:${lastNode.line}` : ''}</code><Link className="quiet-link" href={handoff}>Inspect this path in Lachesis <span aria-hidden="true">↗</span></Link></div>{finding.analysis?.limitations?.length ? <p className="trust-nonclaim">Exporter limitation: {finding.analysis.limitations.join(' ')}</p> : <p className="trust-nonclaim">Presence marks a place to investigate; it does not establish exploitability or a finding.</p>}</div></article>; })}</div> : <p className="trust-empty">No exported evidence matches “{query}”. Try a broader term.</p>}{findings.length === 0 && <p className="trust-empty">This valid snapshot contains no exported security evidence. That is not a claim that the repository is free of security issues; it means this projection has no items to display.</p>}</section>;
}
