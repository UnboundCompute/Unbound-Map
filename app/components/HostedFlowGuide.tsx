...
export function HostedFlowGuide({ bundleId, context, initialFlow, initialStep, route = '/flows' }: { bundleId: string; context: SharedSnapshotContext; initialFlow?: string; initialStep?: string; route?: string }) {
  ...
  const flowStart = flow ? nodeById.get(flow.hops[0]?.node_id)?.label ?? flow.hops[0]?.caption : undefined;
  const flowEnd = flow ? nodeById.get(flow.hops.at(-1)?.node_id ?? '')?.label ?? flow.hops.at(-1)?.caption : undefined;
  ...
  if (!flow) return <section className="map-state-panel" role="status"><span className="map-state-label">No guided path in snapshot</span><h2>This repository does not have a flow to read yet.</h2><p>The graph is valid, but its code-understanding projection did not include a call path. Architecture remains available.</p><div className="map-state-actions"><Link className="primary-button" href={`/architecture?${new URLSearchParams({ repository: bundle!.meta.repository, revision: bundle!.meta.revision, bundle: bundleId }).toString()}`}>Open Architecture <span aria-hidden="true">→</span></Link></div></section>;
  ...
  return <>
    ...
    <section className="guided-flow" aria-labelledby="flow-steps-title">
      ...
      <dl className="flow-fields flow-summary" aria-label="Flow evidence summary"><div><dt>flow type</dt><dd>{flow.kind}</dd></div><div><dt>starts at</dt><dd>{flowStart || 'Not reported'}</dd></div><div><dt>ends at</dt><dd>{flowEnd || 'Not reported'}</dd></div><div><dt>total steps</dt><dd>{flow.hops.length}</dd></div><div><dt>modules represented</dt><dd>{flowModuleCount || 'Not reported'}</dd></div><div><dt>evidence confidence</dt><dd>{flow.confidence || 'Not reported'}</dd></div></dl>
