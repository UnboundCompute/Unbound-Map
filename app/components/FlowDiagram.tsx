...
export function FlowDiagram({ route = '/flows', context = {}, initialStep, initialBranch }: { route?: string; context?: SharedSnapshotContext; initialStep?: string; initialBranch?: string }) {
  ...
  return <section className="guided-flow" aria-labelledby="flow-steps-title">
    ...
    <ol className="flow-path-map" aria-label="Flow path overview">{flowSteps.map((item, index) => <li key={item.id} className={index === active ? 'is-current' : Math.abs(index - active) === 1 ? 'is-adjacent' : undefined}><button type="button" onClick={() => choose(index)} aria-current={index === active ? 'step' : undefined} aria-label={`Select ${item.noun}, step ${index + 1} of ${flowSteps.length}`}><span>{item.regionId}</span><strong>{item.noun}</strong></button>{index < flowSteps.length - 1 && <span className="flow-path-connector" aria-hidden="true">→</span>}</li>)}</ol>
    ...
    <details className="flow-linear" open={linearOpen} onToggle={(event) => setLinearOpen(event.currentTarget.open)}><summary>Read all five handoffs as text</summary><ol>{flowSteps.map((item, index) => <li key={item.id}><button type="button" className={index === active ? 'is-current' : undefined} onClick={() => choose(index)} aria-current={index === active ? 'step' : undefined} aria-controls="flow-step-detail"><strong>{index + 1}. {item.noun}</strong><span>{item.description}</span><small>{item.input} → {item.output}</small></button></li>)}</ol></details>
  </section>;
}
