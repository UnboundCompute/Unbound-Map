...
export function FlowDiagram({ route = '/flows', context = {}, initialStep }: { route?: string; context?: SharedSnapshotContext; initialStep?: string }) {
  ...
  return <section className="guided-flow" aria-labelledby="flow-steps-title">
    ...
    <section className="flow-branches" aria-labelledby="flow-branches-title"><div><span className="flow-label">Branching, kept readable</span><h3 id="flow-branches-title">The handoff has two different kinds of dispatch.</h3></div><div className="flow-branch-grid"><div><strong>Static protocol dispatch</strong><p>EtherType selects the layer-three decoder—IPv4, IPv6, VLAN, ARP, or another supported branch.</p></div><div><strong>Runtime flow dispatch</strong><p>Once a flow exists, state allocation and protocol detection choose parser state from observed traffic.</p></div></div></section>
