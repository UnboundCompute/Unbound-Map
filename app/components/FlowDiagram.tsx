...
function exploreHref(stepId: string, context: SharedSnapshotContext) {
  ...
  return `/explore?${new URLSearchParams({ repository: context.repository ?? illustrativeSnapshot.repository, revision: context.revision ?? illustrativeSnapshot.revision, region: step.regionId, label: step.noun, flow: step.id, anchor: step.anchor, ...(context.bundle ? { bundle: context.bundle } : {}) }).toString()}`;
}
