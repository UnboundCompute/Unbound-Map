...
export function TrustGlossary() {
  ...
  return <section className="trust-glossary" aria-label="Trust domain glossary">
    <div className="trust-tools"><label htmlFor="trust-search">Find a trust domain</label><input id="trust-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search memory, input, lifecycle…" /><label htmlFor="trust-kind">Show</label><select id="trust-kind" value={kind} onChange={(event) => setKind(event.target.value)}><option value="all">All domains</option><option value="input">Input and parsing</option><option value="effect">Effects and cost</option><option value="lifecycle">Lifecycle</option></select></div>
