    ...
    <div className="map-workbench">
      <div className="map-bezel">
        ...
        <div className="map-canvas" aria-label={`${snapshot.repository} architecture map`}>
          ...
          {level === '0' ? regions.map((region, index) => { const position = positionFor(index); return <button key={region.id} className={`map-node node-${position.tone} ${current.id === region.id ? 'is-selected' : ''}`} style={{ left: `${position.x}%`, top: `${position.y}%` }} onClick={() => selectRegion(region)} aria-pressed={current.id === region.id}><span className="node-dot" /><strong>{region.label}</strong><small>{region.metricLabel}</small></button>; }) : <div className="map-focus-content"><span className="map-focus-label">Region focus · {current.label}</span>{current.children?.length ? <ol>{current.children.slice(0, 12).map((child, index) => <li key={child.label}><span>{String(index + 1).padStart(2, '0')}</span><strong>{child.label}</strong><small>{child.summary}</small></li>)}</ol> : <><p>Child projection is not available in this bundle. Open the region chapter for the bounded responsibility and its known anchors.</p><Link className="map-focus-link" href={`/architecture/${current.id}?${new URLSearchParams({ level: '1', repository: snapshot.repository, revision: snapshot.revision, ...(activeBundle ? { bundle: activeBundle } : {}) }).toString()}`}>Open region chapter <span aria-hidden="true">→</span></Link></>}</div>}
