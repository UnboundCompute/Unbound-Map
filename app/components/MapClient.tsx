...
export function MapClient({ route = '/architecture', initialBundle, initialSnapshot, initialLevel = '0', initialRegion = '', initialQuery = '', compact = false, maxRegions = 9, regionIds }: { route?: string; initialBundle?: string; initialSnapshot?: RepositorySnapshotView; initialLevel?: string; initialRegion?: string; initialQuery?: string; compact?: boolean; maxRegions?: number; regionIds?: string[] }) {
  ...
  const internalRelationshipCount = current.internalRelationshipCount ?? 0;
  ...
  const relationshipLede = !edges.length && hasGraphRelationships
    ? `Text view: this bundle includes ${relationshipCount.toLocaleString()} graph relationships${internalRelationshipCount ? `, including ${internalRelationshipCount.toLocaleString()} inside the selected area` : ''}, but no cross-area handoff is inferred at this altitude.`
    : !edges.length
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  ...
  function downloadArchitectureMarkdown() {
    try {
      const source = (file?: string, line?: number) => sourceHref(linkSnapshot, file, line);
      const entrypointLines = (snapshot.entrypoints ?? []).slice(0, 8).map((entry) => {
        const location = `${entry.file}${entry.line ? `:${entry.line}` : ''}`;
        const href = source(entry.file, entry.line);
        return `- **${entry.label}** — ${entry.kind} · ${href ? `[${location}](${href})` : `\`${location}\``}`;
      });
      const regionLines = allRegions.map((region) => {
        const anchor = region.anchor ? ` Anchor: \`${region.anchor.label}\` at \`${region.anchor.file}:${region.anchor.line}\`.` : '';
        const downstream = region.downstream?.length ? ` Hands off to: ${region.downstream.map((id) => allRegions.find((item) => item.id === id)?.label ?? id).join(', ')}.` : '';
        return `### ${region.label}\n\n${region.summary} ${region.metricLabel} · ${region.projectedLabel}. Path: \`${region.path}\`.${anchor}${downstream}`;
      });
      const relationshipLines = allEdges.length
        ? allEdges.map((edge) => `- ${allRegions.find((region) => region.id === edge.from)?.label ?? edge.from} → ${allRegions.find((region) => region.id === edge.to)?.label ?? edge.to}`)
        : ['- No cross-region relationship is inferred at this projection altitude.'];
      const markdown = [
        `# ${linkSnapshot.repository} architecture`,
        '',
        `> Revision-pinned architecture reading guide generated from a graph-backed projection.`,
        '',
        `- **Repository:** ${linkSnapshot.repository}`,
        `- **Revision:** \`${linkSnapshot.revision}\``,
        `- **Evidence:** graph-backed projection; not proof of one observed runtime execution`,
        `- **Coverage:** ${snapshot.coverageScope} · ${snapshot.includedNodes.toLocaleString()} of ${snapshot.indexedNodes.toLocaleString()} indexed nodes included`,
        '',
        '## What this repository does',
        '',
        snapshot.purpose ?? snapshot.description ?? 'Purpose was not supplied by the graph snapshot.',
        '',
        ...
        '## Start reading here',
        '',
        ...(entrypointLines.length ? entrypointLines : ['- No exported entrypoints were included in this snapshot.']),
        '',
        '## Coverage and unresolved regions',
        '',
        ...(snapshot.limitations.length ? snapshot.limitations.map((item) => `- ${item}`) : ['- No additional limitations were reported.']),
        '',
        '## Continue the investigation',
        '',
        `Open the interactive map: ${typeof window === 'undefined' ? '/architecture' : window.location.href}`,
        '',
        'Generated by [Unbound Map](https://map.unboundcompute.com/).',
        '',
      ].join('\n');
      ...
      const slug = linkSnapshot.repository.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'architecture';
      link.href = url;
      link.download = `${slug}-ARCHITECTURE.md`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      URL.revokeObjectURL(url);
  ...
  return <>
    <section className={`map-workbench${compact ? ' is-compact' : ''}`} aria-labelledby="architecture-map-title">
      ...
      {!compact && <aside className="map-inspector" id="architecture-map-detail" aria-live="polite" aria-label="Selected region details"><div className="inspector-label">Selected region</div><h2>{current.label}</h2><p>{current.summary}</p><dl className="inspector-facts"><div><dt>footprint</dt><dd>{current.metricLabel} · {current.projectedLabel}</dd></div><div><dt>path</dt><dd><code>{current.path}</code></dd></div><div><dt>anchor</dt><dd><code>{current.anchor?.label ?? 'No anchor in this projection'}</code></dd></div><div><dt>receives from</dt><dd>{current.upstream?.length ? current.upstream.map((id) => allRegions.find((region) => region.id === id)?.label ?? id).join(', ') : hasGraphRelationships ? internalRelationshipCount ? 'No cross-area input; relationships remain inside this area' : 'No cross-area input reported' : 'Relationship evidence unavailable'}</dd></div><div><dt>hands off to</dt><dd>{current.downstream?.length ? current.downstream.map((id) => allRegions.find((region) => region.id === id)?.label ?? id).join(', ') : hasGraphRelationships ? internalRelationshipCount ? 'No cross-area handoff; relationships remain inside this area' : 'No cross-area handoff reported' : 'Relationship evidence unavailable'}</dd></div>{internalRelationshipCount > 0 && <div><dt>internal relationships</dt><dd>{internalRelationshipCount.toLocaleString()} graph relationship{internalRelationshipCount === 1 ? '' : 's'} within this area</dd></div>}</dl>{(current.semanticSummary || current.tags?.length || current.affinity?.length || typeof current.coherence === 'number') ? <div className="inspector-semantic"><div className="inspector-label">Semantic overlay <span className="semantic-advisory">advisory</span></div>{current.semanticSummary ? <p className="semantic-summary">{current.semanticSummary}</p> : null}{typeof current.coherence === 'number' ? <p className="semantic-coherence"><span>coherence</span> <b>{Math.round(current.coherence * 100)}%</b> <small>how tightly this area&rsquo;s members cluster in meaning</small></p> : null}{current.tags?.length ? <ul className="semantic-tags">{current.tags.slice(0, 8).map((tag) => <li key={tag}>{tag}</li>)}</ul> : null}{current.affinity?.length ? <div className="semantic-affinity"><span>Related areas</span><ul>{current.affinity.slice(0, 4).map((item) => <li key={item.id}>{item.label} <small>{Math.round(item.score * 100)}%</small></li>)}</ul></div> : null}</div> : null}<p className="inspector-note">An anchor is the representative symbol to open first; footprint counts the real declarations this area owns.</p><Link className="inspector-link" href={`/architecture/${regionPath(current.id)}?${new URLSearchParams({ level: '1', repository: linkSnapshot.repository, revision: linkSnapshot.revision, ...(activeBundle ? { bundle: activeBundle } : {}) }).toString()}`}>Read this region <span aria-hidden="true">→</span></Link>{sourceLink && <a className="inspector-link" href={sourceLink} target="_blank" rel="noreferrer">Read source at {current.anchor!.file}:{current.anchor!.line} <span aria-hidden="true">↗</span></a>}<Link className="inspector-link" href={handoffHref(linkSnapshot, current, current.anchor?.label, activeBundle)}>Open anchor in Lachesis <span aria-hidden="true">↗</span></Link></aside>}
      ...
      {!compact && <>{snapshot.entrypoints?.length ? <section className="map-entrypoint-list" aria-labelledby="map-entrypoints-title"><h2 id="map-entrypoints-title">Start points in this snapshot</h2><ul>{snapshot.entrypoints.slice(0, 6).map((entry) => { const href = `/explore?${new URLSearchParams({ repository: linkSnapshot.repository, revision: linkSnapshot.revision, anchor: entry.label, ...(activeBundle ? { bundle: activeBundle } : {}) }).toString()}`; return <li key={entry.id}><Link href={href}>{entry.label} <span aria-hidden="true">↗</span></Link><small>{entry.kind} · {entry.file}{entry.line ? `:${entry.line}` : ''}</small></li>; })}</ul></section> : null}{relationshipEvidence.length > 0 && <p className="relationship-kinds-note">Relationship evidence: {relationshipEvidence.join('; ')}.</p>}<p className="relationship-summary-lede">{relationshipLede}</p><details className="relationship-summary" open><summary>The projection, in words</summary><div className="relationship-summary-content"><p>Use this ordered summary if you prefer reading relationships to navigating a diagram. This list includes bounded regions that may sit outside the placed canvas.</p><ol>{allRegions.map((region) => <li key={region.id}><button type="button" onClick={() => selectRegion(region)} aria-pressed={current.id === region.id} aria-controls="architecture-map-detail"><span>{region.label}</span><small>{region.downstream?.length ? `hands off to ${region.downstream.map((id) => allRegions.find((item) => item.id === id)?.label ?? id).join(', ')}` : !edges.length ? hasGraphRelationships ? 'graph relationships stay within this projected region' : 'relationship evidence unavailable' : region.role === 'boot' ? 'initializes the runtime' : 'ends the displayed path'}</small></button></li>)}</ol></div></details>{snapshot.enrichment && (snapshot.enrichment.readingOrder?.length || snapshot.enrichment.crossCutting?.length || snapshot.enrichment.nearDuplicates?.length) ? <section className="semantic-overlay" aria-labelledby="semantic-overlay-title"><div className="semantic-overlay-head"><span className="thesis-label">Semantic overlay · advisory</span><h2 id="semantic-overlay-title">How the code reads, by meaning</h2><p>An optional embedding pass arranged and labeled these areas by textual similarity. It annotates the graph-backed structure above &mdash; it never redefines it, and it is absent when no model was present at export time. Treat it as a reading aid, not evidence.{snapshot.enrichment.model ? ` Model: ${snapshot.enrichment.model}.` : ''}</p></div>{snapshot.enrichment.readingOrder?.length ? <div className="semantic-block"><h3>Suggested reading order</h3><ol className="semantic-tour">{snapshot.enrichment.readingOrder.map((step) => { const target = allRegions.find((region) => region.id === step.id); return <li key={step.id}>{target ? <button type="button" onClick={() => selectRegion(target)}>{step.label}</button> : <span>{step.label}</span>}</li>; })}</ol></div> : null}{snapshot.enrichment.crossCutting?.length ? <div className="semantic-block"><h3>Cross-cutting concerns</h3><ul className="semantic-crosscut">{snapshot.enrichment.crossCutting.slice(0, 8).map((concern) => <li key={concern.tag}><strong>{concern.tag}</strong><small>{concern.count.toLocaleString()} function{concern.count === 1 ? '' : 's'} across {concern.moduleSpread} area{concern.moduleSpread === 1 ? '' : 's'}</small></li>)}</ul></div> : null}{snapshot.enrichment.nearDuplicates?.length ? <div className="semantic-block"><h3>Near-duplicate members</h3><ul className="semantic-dupes">{snapshot.enrichment.nearDuplicates.slice(0, 6).map((pair, index) => <li key={`${pair.aHint ?? pair.aLabel}-${pair.bHint ?? pair.bLabel}-${index}`}><span>{pair.aLabel}{pair.aHint ? <span className="dup-loc"> {pair.aHint}</span> : null} <em>&harr;</em> {pair.bLabel}{pair.bHint ? <span className="dup-loc"> {pair.bHint}</span> : null}</span><small>{Math.round(pair.score * 100)}% similar</small></li>)}</ul></div> : null}</section> : null}</>}
  </>;
}
