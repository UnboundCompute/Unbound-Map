...
export function MapClient({ route = '/architecture', initialBundle, initialLevel = '0', initialRegion = 'decode', initialQuery = '' }: { route?: string; initialBundle?: string; initialLevel?: string; initialRegion?: string; initialQuery?: string }) {
  ...
  if (!current) {
    return <section className="map-state-panel" role="status" aria-live="polite" aria-atomic="true"><span className="map-state-label">No regions in snapshot</span><h2>There is no architecture to draw yet.</h2><p>This snapshot is valid but contains no displayable top-level regions. Return to the repository start page or open the source explorer for coverage details.</p><Link className="quiet-link" href={startHref}>Return to Start here <span aria-hidden="true">→</span></Link></section>;
  }
