...
import { illustrativeSnapshot, snapshotFromProjection, snapshotWithContext, sourceHref, toHandoff, type RepositorySnapshotView, type SystemRegion } from '../../lib/view-model';
...
export function MapClient({ route = '/architecture', initialBundle, initialLevel = '0', initialRegion = 'decode', initialQuery = '', compact = false, maxRegions = 9, regionIds }: { route?: string; initialBundle?: string; initialLevel?: string; initialRegion?: string; initialQuery?: string; compact?: boolean; maxRegions?: number; regionIds?: string[] }) {
  ...
  const relationshipLede = snapshot.provenance === 'graph-backed' && !edges.length && hasGraphRelationships
    ...
    : `Text view: wire input enters ${entryRegion?.label ?? 'the first region'}; the bounded regions hand work forward, and ${outputRegion?.label ?? 'the final region'} hands alerts and telemetry to consumers.`;
  ...
  return <>
    <section className={`map-workbench${compact ? ' is-compact' : ''}`} aria-labelledby="architecture-map-title">
      ...
      <div className="map-bezel">
        ...
        {bundleState === 'ready' && snapshot.description && <p className="map-thesis">{snapshot.description}</p>}
