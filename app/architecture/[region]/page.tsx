...
import { snapshotFromProjection } from '../../../lib/view-model';
...
export default async function RegionPage({ params, searchParams }: { params: Promise<{ region: string }>; searchParams: SearchParams }) {
  ...
  if (context.bundle) {
    try {
      ...
      const projected = projectTopLevelRegions(toDesignMapSnapshot(payload));
      snapshot = snapshotFromProjection(toDesignMapSnapshot(payload), projected);
    } catch (error) {
