...
export default async function ArchitecturePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <DocsShell active="/architecture" snapshot={snapshot} context={context}><div className="doc-page architecture-page"><PageIntro title="What are the major responsibilities?" snapshot={snapshot}>{architectureLede}</PageIntro><MapClient initialBundle={context.bundle} initialLevel={initialLevel} initialRegion={one(query.region) ?? ''} initialQuery={initialQuery ? `?${initialQuery}` : ''} /><EmbedSnippet context={context} /><section className="architecture-next"><h2>Map before source.</h2><p>Placement is a reading aid. Labels, counts, and anchors come from the loaded graph snapshot; exact symbol behavior belongs in Lachesis.</p><Link className="quiet-link" href={`/explore?${contextQuery}`}>Continue to Lachesis with this snapshot <span aria-hidden="true">↗</span></Link></section><EvidenceNote>A graph-backed bundle was requested. The map stays hidden until validation succeeds; regions, counts, relationships, and coverage come from that snapshot.</EvidenceNote></div></DocsShell>;
}
