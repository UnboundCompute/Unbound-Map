...
export default async function ArchitecturePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <DocsShell active="/architecture" snapshot={snapshot}><div className="doc-page architecture-page"><PageIntro title="What are the major responsibilities?" snapshot={snapshot}>The first map is deliberately bounded. It shows the regions that carry {snapshot.repository} from network input to detection and output, then lets you focus one region at a time.</PageIntro><MapClient /><EmbedSnippet /><section className="architecture-next"><h2>Map before source.</h2><p>Placement is a reading aid. Labels, counts, and anchors should come from the loaded graph snapshot; exact symbol behavior belongs in Lachesis.</p><Link className="quiet-link" href={`/flows${contextQuery ? `?${contextQuery}` : ''}`}>Follow the packet flow <span aria-hidden="true">→</span></Link></section><EvidenceNote>Illustrative prototype: this eight-region map is editorial fixture content. A graph-backed bundle may produce a different shape, number of regions, and coverage scope.</EvidenceNote></div></DocsShell>;
}
