...
export default async function TrustPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <DocsShell active="/trust" snapshot={snapshot} context={context}><div className="doc-page trust-page"><PageIntro title={context.bundle ? 'What evidence needs a closer read?' : 'Where do obligations begin?'} snapshot={snapshot}>{context.bundle ? 'This index keeps exported graph evidence separate from adjudicated findings. Open the source context before drawing a security conclusion.' : 'Trust surfaces are places where data enters, is constrained, or leaves a processing boundary. This page explains the obligation without turning a source or sink into a vulnerability claim.'}</PageIntro>{context.bundle ? <><HostedTrustGlossary bundleId={context.bundle} context={context} initialQuery={one(query.q)} /><EvidenceNote>Graph-backed evidence is a pointer into Lachesis, not a vulnerability verdict. The illustrative taxonomy is intentionally not substituted for a repository snapshot.</EvidenceNote></> : <><section className="trust-intro"><h2>Read the obligation, then inspect the evidence.</h2><p>Choose a surface by what it does—enters, protects, or leaves—then open its anchor in Lachesis for exact guards and references.</p></section><TrustGlossary context={context} initialQuery={one(query.q)} initialKind={initialKind} initialDomain={one(query.domain)} /><EvidenceNote>Illustrative taxonomy: the domains and locations below are a prototype index. Presence identifies where to investigate; it does not establish exploitability or a finding.</EvidenceNote><Link className="quiet-link" href={`/architecture${contextQuery ? `?${contextQuery}` : ''}`}>Return to Architecture <span aria-hidden="true">→</span></Link></>}</div></DocsShell>;
}
